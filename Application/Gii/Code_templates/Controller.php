...
class <?php echo $tpName; ?>Controller extends AdminController 
{
    public function add()
    {
    	if(IS_POST)
...
<?php foreach ($config['fields'] as $k => $v):if(isset($v['validate']['unique'])): ?>
    public function ajax_chk_<?php echo $k; ?>_unique()
    {
    	$<?php echo $k; ?> = I('get.<?php echo $k; ?>');
    	$<?php echo $config['pk']; ?> = I('get.<?php echo $config['pk']; ?>', '');
    	if($<?php echo $config['pk']; ?>)
    		$<?php echo $config['pk']; ?> = "<?php echo $config['pk']; ?> <> $<?php echo $config['pk']; ?> AND ";
		$model = M('<?php echo $tpName; ?>');
		$count = $model->where("$<?php echo $config['pk']; ?> <?php echo $k; ?> = '$<?php echo $k; ?>'")->count();
		if($count == 0)
			echo 'true';
		else 
			echo 'false';
